...
function debug_log(...output) {
  console.log("%cdiscordify", Object.entries(LOG_STYLES).map(([a, b]) => `${a}:${b};`).join(""), ...output);
}
...
var Discordify = class {
  ...
  start() {
    ...
    LOGS.forEach((l) => Dispatcher.subscribe(l, (e) => debug_log(l, e)));
  }
  stop() {
    BdApi.Patcher.unpatchAll("discordify");
    LOGS.forEach((l) => Dispatcher.unsubscribe(l, (e) => debug_log(l, e)));
  }
};
var MainComponent = () => {
